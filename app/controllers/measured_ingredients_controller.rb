class MeasuredIngredientsController < ApplicationController
  before_action :set_measured_ingredient, only: %i[ show edit update destroy ]

  # GET /measured_ingredients or /measured_ingredients.json
  def index
    @measured_ingredients = MeasuredIngredient.all
  end

  # GET /measured_ingredients/1 or /measured_ingredients/1.json
  def show
  end
  ...
  # POST /measured_ingredients or /measured_ingredients.json
  def create
  ...
  # PATCH/PUT /measured_ingredients/1 or /measured_ingredients/1.json
  def update
    respond_to do |format|
  ...
  # DELETE /measured_ingredients/1 or /measured_ingredients/1.json
  def destroy
  ...
  private
    ...
    def measured_ingredient_params
      params.require(:measured_ingredient).permit(:amount, :dish_id, :ingredients_id, :measurements_id)
    end
end
