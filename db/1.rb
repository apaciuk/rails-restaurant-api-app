MEASUREMENTS = %i[millilitre litre milligram gram].freeze
...
NUMBER_OF_RESTAURANTS.times do
  restaurant = Restaurant.create(name: Faker::Restaurant.name, description: Faker::Lorem.characters(number: 10))

  DISHES_PER_RESTAURANT.times do
    dish = restaurant.dishes.create(name: Faker::Food.unique.dish)
    #downloaded_image = URI.call("https://source.unsplash.com/600x600/?#{dish.name}")
